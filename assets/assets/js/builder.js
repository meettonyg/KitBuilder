...
(function(window, document) {
    ...
    function initBuilder() {
        ...
        if (typeof jQuery === 'undefined') {
            console.error('Media Kit Builder: jQuery is required but not loaded');
            return;
        }

        // Check if mkb_ajax is available
        if (typeof mkb_ajax === 'undefined') {
            console.error('Media Kit Builder: mkb_ajax localization data is missing');
            return;
        }
    ...
    function checkAndInit() {
        if (typeof jQuery !== 'undefined' && document.readyState !== 'loading') {
            // Remove loading screen once builder is initialized
            jQuery(document).ready(function() {
                console.log('Initializing Media Kit Builder...');
                try {
                    window.mkBuilder = new MediaKitBuilder();
                    // Hide loading screen
                    const loadingScreen = document.querySelector('.mkb-loading-screen');
                    if (loadingScreen) {
